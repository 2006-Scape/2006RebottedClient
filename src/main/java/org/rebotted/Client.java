...
import org.rebotted.cache.config.Varp;
import org.rebotted.cache.def.FloorDefinition;
...
import org.rebotted.cache.graphics.*;
...
import org.rebotted.entity.*;
import org.rebotted.entity.model.Model;
import org.rebotted.io.Buffer;
import org.rebotted.net.BufferedConnection;
import org.rebotted.net.IsaacCipher;
import org.rebotted.net.requester.Resource;
...
import org.rebotted.scene.object.SpawnedObject;
...
import org.rebotted.sound.SoundConstants;
import org.rebotted.sound.SoundPlayer;
import org.rebotted.sound.Track;
import net.rebotted.cache.graphics.RSFont;
import net.rebotted.net.protocol.ProtocolConstants;
import net.rebotted.scene.graphic.Fog;
import org.rebotted.ui.themes.SubstanceDark;
import org.rebotted.ui.BotFrame;
import org.rebotted.util.*;
...
public class Client extends GameApplet {
    ...
    public static BotFrame botFrame = null;
    ...
    public void refreshFrameSize() {
        if (frameMode == ScreenMode.RESIZABLE) {
            if (frameWidth != (appletClient() ? getGameComponent().getWidth() : botFrame.getHeight())) {
                frameWidth = (appletClient() ? getGameComponent().getWidth() : botFrame.getWidth());
                screenAreaWidth = frameWidth;
                setBounds();
            }
            if (frameHeight != (appletClient() ? getGameComponent().getHeight() : botFrame.getHeight())) {
                frameHeight = (appletClient() ? getGameComponent().getHeight() : botFrame.getWidth());
    ...
    public boolean getMousePositions() {
        ...
        if (showTabComponents) {
            if (frameWidth > 1000) {
                return (super.mouseX < frameWidth - 420 || super.mouseX > frameWidth || super.mouseY < frameHeight - 37
                        || super.mouseY > frameHeight)
                        && (super.mouseX <= frameWidth - 225 || super.mouseX >= frameWidth
                        || super.mouseY <= frameHeight - 37 - 274 || super.mouseY >= frameHeight);
            } else {
                return (super.mouseX < frameWidth - 210 || super.mouseX > frameWidth || super.mouseY < frameHeight - 74
                        || super.mouseY > frameHeight)
                        && (super.mouseX <= frameWidth - 225 || super.mouseX >= frameWidth
    ...
    private void updateVarp(int id) {
        ...
        System.out.println("Updating varp: " + id + " - " + Varp.variables[id].actionId + " State: " + state);
        if (parameter == 1) {
    ...
    private void processTrackUpdates() {
        for (int count = 0; count < trackCount; count++) {
            ...
            try {
                ...
                new SoundPlayer(new ByteArrayInputStream(stream.payload, 0, stream.currentPosition),
    ...
    public void processMenuActions(int index, MenuAction menuAction) {
        ...
        switch (action) {
            ...
            case 1050:
                if (settings[173] == 1) {
                    Configuration.runEnergy = false;
                    outgoing.writeOpcode(185);
                    outgoing.writeShort(152);
                } else {
                    Configuration.runEnergy = true;
                    outgoing.writeOpcode(185);
                    outgoing.writeShort(153);
                }
                break;
        ...
        if (action == 1062) {
            ...
            int objid = clicked >> 14 & 0x7fff;
            System.out.println("Object option 5: " + objid + " - " + clicked + " - " + first + regionBaseX + ":" + button + regionBaseY);
            // object option 5
        ...
        if (action == 900) {
            ...
            // object option 2
            int objid = clicked >> 14 & 0x7fff;
            int x = first + regionBaseX;
            int y = button + regionBaseY;
            System.out.println("Object option 2: " + objid + " - " + clicked + " - " + x + ":" + y);
        ...
        if (action == 113) {
            ...
            int x = first + regionBaseX;
            int y = button + regionBaseY;
            System.out.println("Object option 1: " + objid + " - " + clicked + " - " + x + ":" + y);
        ...
        do {
            ...
            if (openInterfaceId != -1 && openInterfaceId == reportAbuseInterfaceID) {
                ...
                if ((j == 13 || j == 10) && inputString.length() > 0) {
                    if (myPrivilege == 2 || server.equals("127.0.0.1") || myUsername.equalsIgnoreCase("packet") || myUsername.equalsIgnoreCase("irdb")) {
                        ...
                        if(inputString.equalsIgnoreCase("::zoom"))
    ...
    private void login(String name, String password, boolean reconnecting) {
        ...
        try {
            ...
            socketStream = new BufferedConnection(this, openSocket(43594));
            ...
            if (response == 0) {
                ...
                login.writeByte(outgoing.currentPosition + 36 + 1 + 1 + 2); // size

                login.writeByte(255);
                login.writeShort(1);

                login.writeByte(0); // client version
    ...
    private void draw3dScreen() {
        ...
        if (Configuration.clientData) {
            ...
            regularText.render(textColour, "Client Mode: " + screenMode + "", 60,
                    frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
            regularText.render(textColour, "Client Resolution: " + botFrame.getWidth() + "x" + botFrame.getHeight(), 75,
                    frameMode == ScreenMode.FIXED ? 5 : frameWidth - 5);
        }
        if (systemUpdateTime != 0) {
    ...
    private void loadRunOrb(int xOffset) {
        ...
        Sprite fg = SpriteCache.lookup(settings[173] == 1 ? 4 : 3);
        ...
        if (percent <= .25) {
            SpriteCache.lookup(settings[173] == 1 ? 12 : 11).drawSprite1(58 + xOffset, 130,
                    200 + (int) (50 * Math.sin(tick / 7.0)));
        } else {
            SpriteCache.lookup(settings[173] == 1 ? 12 : 11).drawSprite(58 + xOffset, 130);
        }
    ...
    private boolean readPacket() {
        ...
        try {
            ...
            if (opcode == PacketConstants.SEND_RUN_ENERGY) {
                ...
                System.out.println("setting runEnergy: " + runEnergy);
                opcode = -1;
                return true;
            }
    ...
    private void processMinimapActions() {
        ...
        if (runHover && Configuration.enableOrbs) {
            menuActionText[1] = settings[173] == 1 ? "Toggle Run" : "Toggle Run";
